...
const Step3 = (props) => {
  ...
  const [highlight_field, setHighlight_field] = useState([
    { index: 0, highlights: "" },
  ]);
  const [facts, setFacts] = useState([]);

  const [temple_history, settemple_history] = useState([
    { index: 0, year: null, detail: null },
  ]);
  const [history, setHistory] = useState([])

  const addHighLight = (key) => {
    const rows = [...highlight_field];
    rows.push({
      index: key + 1,
      highlights: "",
    });
    setHighlight_field(rows);
  };

  const updateHightLight = (value, idx) => {
    const rows = [...highlight_field];
    rows[idx].highlights = value;
    setHighlight_field(rows);
    let prefacts = [];
    highlight_field.forEach((element) => {
      prefacts.push(element.highlights);
    });
    setFacts(prefacts);
  };

  const minusHighLights = (key) => {
    let ab = highlight_field.filter((item) => item.index !== key);
    setHighlight_field(ab);
  };
  ...
  const step4 = async () => {
    try {
      ...
      const outputData = JSON.stringify(history);
      await AsyncStorage.setItem('itemListed', outputData)
      props.navigation.navigate("createPoojariStep4");
    } catch (error) {
      console.log(error)
    }
  }
      ...
      <View style={styles.mainHeaderOuter}>
        <LinearGradient
          ...
          <View style={styles.backMenuDiv}>
            <TouchableOpacity style={styles.backBtnDiv} onPress={() => props.navigation.navigate("createPoojariStep2")}>
              <Image source={require('../img/rightArrow1.jpg')} style={[styles.arrowBtn, styles.arrowBtnLight]} />
            </TouchableOpacity>
          </View>
          <Text style={styles.headerTitle}>Highlights & History</Text>
        </LinearGradient>
      </View>
                  ...
                  <View style={[styles.row, styles.pt10,]} >
                    <View style={[styles.spaceBetween]}>
                      ...
                      <View style={[styles.MainTitleDiv, styles.flatOtrIconBx]}>
                        <Text style={[styles.mainTitleText2, styles.textEarthyRed]}>Highlights</Text>
                      </View>
                    </View>
                  </View>
                </View>

                {highlight_field.map((data, key) => {
                  return (
                    <View key={key} style={[styles.row, styles.spaceBetween, styles.mt20]} >
                      {/* <View style={[styles.row, styles.spaceBetween]} > */}
                      <TextInput
                        style={styles.formControlNewthree}
                        value={data.highlights}
                        onChangeText={(value) => updateHightLight(value, key)}
                        placeholder="Highlights"
                        placeholderTextColor="#888"
                      />
                      {key == 0 ? (
                        <TouchableOpacity onPress={() => addHighLight(highlight_field.length - 1)}>
                          <Image source={require('../img/plus_circle.png')} style={[styles.svgResize]} />
                        </TouchableOpacity>
                      ) : (
                        <TouchableOpacity onPress={() => minusHighLights(data.index)}>
                ...
                <View style={styles.formGroupDiv}>
                  <View style={styles.formGroupNew}>
                    <View style={[styles.MainTitleDiv]}>
                      <Text style={[styles.mainTitleText2, styles.textEarthyRed]}>Pujari History </Text>
                    </View>
                  </View>
                </View>
