...
const SearchTempleList = (props) => {
  ...
  useEffect(() => {
    ...
    const getList = async () => {
      try {
        ...
        await axios({
          method: 'post',
          url: `${BASE_URL_APP}/temple/filter?page=${pageNo}&limit=10`,
          data: data
        }).then(res => {
          // console.log('response',res?.data);
          setTempleList(res?.data?.results?.temples);
    ...
    try {
      if (pageNo > 1) {
        ...
        axios({
          ...
          setTempleList(prev => [...prev, ...res?.data?.results?.temples]);
  ...
  const onSearch = async (text) => {
    try {
      ...
      await axios({
        method: 'post',
        url: `${BASE_URL_APP}/temple/filter?page=1&limit=10`,
        data: { name: text },
      }).then(res => {
        console.log(res?.data);
        setFilteredDate(res?.data?.results?.temples);
        setTotalRecords(res?.data?.totalResults);
        if (res?.data?.results?.next) {
          setHasNext(true);
        } else {
        ...
        <ScrollView contentContainerStyle={styles.outerContainer}>
          <View style={styles.mainBoxOuter}>
            <View
              ...
              <TempleMap templeList={templeList} search={search} name="temple" navigation={props.navigation} />
            </View>
              ...
              <View style={{ marginHorizontal: '3%' }}>
                {search === '' ?
                  <FlatList
                    ...
                    data={templeList}
                    ListFooterComponent={
                      templeList?.length > 0 && hasNext && renderFooter()
                    }
                    renderItem={({ item }) => {
                      return (
                        <TouchableOpacity onPress={() => onCardClick(item._id)}>
                          <ListCard
                            imagePath={item.templeImages.templePictures[0]}
                            name={item.title}
                  ...
                  <View
                    ...
                    <View
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          let temList = templeList.sort((a, b) =>
                            a.title > b.title ? 1 : -1,
                          );
                          setTempleList(temList);
                          setVisible(false);
                        }}>
...
export default SearchTempleList;
