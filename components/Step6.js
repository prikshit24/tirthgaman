...
const Step3 = (props) => {
  ...
  const [temple_video, setTemple_video] = useState([
    { index: 0, title: null, videoLink: null },
  ]);
  const [video, setVideo] = useState([])

  const addvideo = (key) => {
    const rows = [...temple_video];
    rows.push({
      index: key + 1,
      title: "",
      videoLink: "",
    });
    setTemple_video(rows);
  };

  const updatevideo = (value, idx) => {
    const rows = [...temple_video];
    rows[idx].title = value;
    setTemple_video(rows);
  };

  const updateTitle = (value, idx) => {
    const rows = [...temple_video];
    rows[idx].videoLink = value;
    setTemple_video(rows);
    const prevideo = []
    temple_video.forEach((element) => {
      prevideo.push({
        title: element.title,
  ...
  const minusvideo = (key) => {
    let ab = temple_video.filter((item) => item.index !== key);
    setTemple_video(ab);
  };
  ...
  const step4 = async () => {
    try {
      const videoInput = JSON.stringify(video);
      await AsyncStorage.setItem('video', videoInput)
                ...
                <View style={styles.formGroupDiv}>
                  <View style={styles.formGroupNew}>
                    <View style={[styles.MainTitleDiv]}>
                      <Text style={[styles.mainTitleText2, styles.textEarthyRed]}>Upload Video Link</Text>
                    </View>
                  </View>
                </View>
                {temple_video.map((data, key) => {
                  return (
                    <View key={key} style={[styles.row, styles.spaceBetween, styles.mt20]} >
                      ...
                      <View style={{ maxWidth: '90%', margin: 0, padding: 0 }}>
                        ...
                        <TextInput
                          ...
                          onChangeText={(value) => updateTitle(value, key)}
                          placeholder="Title"
                        ...
                        <TextInput
                          ...
                          value={data.title}
                          onChangeText={(value) => updatevideo(value, key)}
                          placeholder="Video"
                          placeholderTextColor="#888"
                        />
                      ...
                      {key == 0 ? (
                        <TouchableOpacity onPress={() => addvideo(temple_video.length - 1)}>
                          <Image source={require('../img/plus_circle.png')} style={[styles.svgResize]} />
                        </TouchableOpacity>
                      ) : (
                        <TouchableOpacity onPress={() => minusvideo(data.index)}>
